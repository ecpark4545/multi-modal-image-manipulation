...
# ################## Loss for G and Ds ##############################
def discriminator_loss(netD, real_imgs, new_real_imgs, fake_imgs, conditions,
                       real_labels, fake_labels, words_embs, cap_lens, image_encoder, class_ids,
                        w_words_embs, wrong_caps_len, wrong_cls_id):
    # Forward
    real_features = netD.D_feature(real_imgs)
    new_real_features = netD.D_feature(new_real_imgs)
    fake_features = netD.D_feature(fake_imgs.detach())
    # loss
    # ================= Discriminator Condition Loss ====================
    cond_real_logits = netD.D_condition(real_features, conditions)
    cond_real_errD = nn.BCELoss()(cond_real_logits, real_labels)
    cond_fake_logits = netD.D_condition(fake_features, conditions)
    cond_fake_errD = nn.BCELoss()(cond_fake_logits, fake_labels)
    # ================== Discriminator Matching Loss ====================
    match_real_logits = netD.D_match(real_features, real_features, conditions)
    match_real_errD = nn.BCELoss()(match_real_logits, real_labels)

    match_fake_logits = netD.D_match(real_features, fake_features, conditions)
    match_fake_errD = nn.BCELoss()(match_fake_logits, fake_labels)

    match_wrong_logtis1 = netD.D_match(real_features, new_real_features, conditions)
    match_wrong_errD1 = nn.BCELoss()(match_wrong_logtis1, fake_labels)
    # ==================================================================
    batch_size = real_features.size(0)
    cond_wrong_logits = netD.D_condition(real_features[:(batch_size - 1)], conditions[1:batch_size])
    cond_wrong_errD = nn.BCELoss()(cond_wrong_logits, fake_labels[1:batch_size])

    if netD.D_adv is not None:
        real_logits = netD.D_adv(real_features)
        fake_logits = netD.D_adv(fake_features)
    ...
    errD += (match_real_errD + match_fake_errD + match_wrong_errD1) / 3.
...
def generator_loss(netsD, image_encoder, fake_imgs, real_labels,
    ...
    for i in range(numDs):
        real_features = netsD[i].D_feature(real_imgs[i])
        fake_features = netsD[i].D_feature(fake_imgs[i])  
        cond_logits = netsD[i].D_condition(fake_features, sent_emb)
        cond_errG = nn.BCELoss()(cond_logits, real_labels)
        match_logtis = netsD[i].D_match(real_features, fake_features, sent_emb)
        match_errG = nn.BCELoss()(match_logtis, real_labels)

        if netsD[i].D_adv is  not None:   
            logits = netsD[i].D_adv(fake_features)
            errG = nn.BCELoss()(logits, real_labels)
            g_loss = errG + cond_errG
        else:
            g_loss = cond_errG
        errG_total += g_loss + match_errG
